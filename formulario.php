...
if ($stmtFormulario->rowCount() > 0) {
    ...
    if ($formulario['rascunho_data']) {
        $rascunhoData = json_decode($formulario['rascunho_data'], false);
        $rascunhoCarregado = false;
    }
}
...
if ($isUpload) {
    if (!empty($_FILES['arquivo']['name']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $destino)) {
            ...
            $_SESSION['mensagem'] = "Erro ao mover arquivo!";
        }
    } else {
        $_SESSION['mensagem'] = "Nenhum arquivo selecionado ou erro no upload!";
    }
    // Evita processar mais nada no upload
    header("Location: " . $_SERVER['REQUEST_URI']);
    exit;
}

// ======================== PROCESSAR FORMULÁRIO ========================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$isUpload) {
    $acao = $_POST['acao'] ?? '';

    if ($acao === 'salvar_rascunho') {
        // Salva rascunho manualmente (não automático)
        $rascunhoData = [
        ...
        if ($formulario) {
            $stmtUpdate = $db->prepare("UPDATE formularios SET rascunho_data = :rascunho_data WHERE id = :id");
            $stmtUpdate->bindParam(':rascunho_data', $rascunhoJson);
            $stmtUpdate->bindParam(':id', $formulario['id']);
            $stmtUpdate->execute();
        } else {
            $stmtInsert = $db->prepare("INSERT INTO formularios (usuario_id, rascunho_data) VALUES (:usuario_id, :rascunho_data)");
            $stmtInsert->bindParam(':usuario_id', $_SESSION['usuario_id']);
            $stmtInsert->bindParam(':rascunho_data', $rascunhoJson);
            $stmtInsert->execute();
        }

        $sucesso = ($acao === 'salvar_rascunho') ? 'Rascunho salvo com sucesso!' : '';
    }

    // Aqui mantém sua lógica de enviar_formulario exatamente como já estava
    // ...
}
...
<body>
    ...
    function uploadFiles(event) {
        if (event) event.preventDefault(); // Previne o envio do formulário
